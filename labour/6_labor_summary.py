...
INPUT_PATH = "/Users/nikhil/Documents/Thesis/Labour/EMP_figaro2025.xlsx"
COUNTRY_OUTPUT_PATH = "/Users/nikhil/Documents/Thesis/Labour/Labour_Country_Summary.xlsx"
INDUSTRY_OUTPUT_PATH = "/Users/nikhil/Documents/Thesis/Labour/Labour_Industry_Summary.xlsx"
...
def make_country_summaries(df: pd.DataFrame):
    ...
    # Order rows by 2023 descending
    global_share = global_share.sort_values(by="2023", ascending=False)
    ...
    eu_share = eu_share.sort_values(by="2023", ascending=False)
...
def make_industry_summaries(df: pd.DataFrame):
    ...
    global_ind_share = global_ind_share.sort_values(by="2023", ascending=False)
    ...
    eu_ind_share = eu_ind_share.sort_values(by="2023", ascending=False)
...
def write_excel_country(global_share: pd.DataFrame, eu_share: pd.DataFrame, out_path: Path):
    with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
        ...
        global_share.to_excel(writer, sheet_name="Global_Labour_Share")
        # Write EU_Labour_Share
        eu_share.to_excel(writer, sheet_name="EU_Labour_Share")


def write_excel_industry(global_ind_share: pd.DataFrame, eu_ind_share: pd.DataFrame, out_path: Path):
    with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
        global_ind_share.to_excel(writer, sheet_name="Global_Labour_Industry_Share")
        eu_ind_share.to_excel(writer, sheet_name="EU_Labour_Industry_Share")
