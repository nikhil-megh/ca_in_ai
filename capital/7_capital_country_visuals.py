...
# ---------- CONFIG ----------
INPUT_XLSX = "/Users/nikhil/Documents/Thesis/Capital/Descriptives_Country/Capital_Country_Summary.xlsx"
OUTPUT_DIR = "/Users/nikhil/Documents/Thesis/Capital/Descriptives_Country/Visuals"

# Sheet names (adjust if your workbook uses different names)
SHEET_GLOBAL_CAPITAL_SHARE = "Global_Capital_Share"
SHEET_EU_CAPITAL_SHARE = "EU_Capital_Share"
...
def line_trend_for_countries(df_wide, country_list, years, title, outpath):
    ...
    plt.xlabel("Year")
    plt.ylabel("Capital share (fraction)")
    plt.title(title)
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
...
def generate_all_charts(input_xlsx, output_dir):
    ...
    sheets = {}
    for name in [SHEET_GLOBAL_CAPITAL_SHARE, SHEET_EU_CAPITAL_SHARE]:
    ...
    # --- Global Capital Share: pie (top9) + line trend for top9 ---
    df_global_capital_share = sheets[SHEET_GLOBAL_CAPITAL_SHARE]
    if df_global_capital_share is not None:
        # ensure columns as ints, pick 2021 column
        if 2021 not in df_global_capital_share.columns:
            raise ValueError("Global_Capital_Share sheet must contain a 2021 column")
        s2021 = df_global_capital_share[2021].astype(float)
        ...
        pie_top_n_with_others(s2021, top_n=9,
                              title="Global Capital Share 2021 (Top 9)",
                              outpath=os.path.join(output_dir, "Global_Capital_Top9_2021_pie.png"))
        ...
        df_global_capital_wide = df_global_capital_share.copy().astype(float)
        # If sheet columns go back to 2000, we pick 2010-2021
        years_present = [y for y in YEARS if y in df_global_capital_wide.columns]
        line_trend_for_countries(df_global_capital_wide, top9, years_present,
                                 title="Global Capital Share Trend 2010-2021 (Top 9)",
                                 outpath=os.path.join(output_dir, "Global_Capital_Top9_trend_2010_2021.png"))
    else:
        print("Skipping Global_Capital_Share plots because sheet not found.")

    # --- EU Capital Share: pie (top9) + line trend for top9 ---
    df_eu_capital_share = sheets[SHEET_EU_CAPITAL_SHARE]
    if df_eu_capital_share is not None:
        if 2021 not in df_eu_capital_share.columns:
            raise ValueError("EU_Capital_Share sheet must contain a 2021 column")
        s2021_eu = df_eu_capital_share[2021].astype(float)
        s2021_eu = normalize_shares_series(s2021_eu)

        pie_top_n_with_others(s2021_eu, top_n=9,
                              title="EU Capital Share 2021 (Top 9)",
                              outpath=os.path.join(output_dir, "EU_Capital_Top9_2021_pie.png"))

        top9 = list(s2021_eu.sort_values(ascending=False).index[:9])
        df_eu_capital_wide = df_eu_capital_share.copy().astype(float)
        years_present = [y for y in YEARS if y in df_eu_capital_wide.columns]
        line_trend_for_countries(df_eu_capital_wide, top9, years_present,
                                 title="EU Capital Share Trend 2010-2021 (Top 9)",
                                 outpath=os.path.join(output_dir, "EU_Capital_Top9_trend_2010_2021.png"))
    else:
        print("Skipping EU_Capital_Share plots because sheet not found.")
