...
DONORS_BUCKET2 = {
    ...
    "TR": ["GR", "ES", "RO"],
    "SA": ["NO", "AU", "RU"],
    "ID": ["BG", "CZ", "SK", "LV", "EE", "LT", "HR", "HU", "PL", "RO", "SI", "GR"],
    "ZA": ["BG", "CZ", "SK", "LV", "EE", "LT", "HR", "HU", "PL", "RO", "SI", "GR"],
    "FIGW1": ["BG", "CZ", "SK", "LV", "EE", "LT", "HR", "HU", "PL", "RO", "SI", "GR"],
    "MX": ["US", "BG", "CZ", "SK", "LV", "EE", "LT", "HR", "HU", "PL", "RO", "SI", "GR"],
    "BR": ["MX", "IN", "BG", "CZ", "SK", "LV", "EE", "LT", "HR", "HU", "PL", "RO", "SI", "GR"],
    "AR": ["BR"],
}
...
def process_country(excel_path, country_code, all_processed):
    cap_raw_np = pd.read_excel(excel_path, sheet_name=country_code, usecols="C:N",
                               skiprows=11, nrows=91, header=None).to_numpy()
    va_raw_np = pd.read_excel(excel_path, sheet_name=country_code, usecols="Q:AB",
...
# -------------------------
# PROCESSING FOR BUCKET 2 (ONLY TOTAL AVAILABLE)
# -------------------------
def process_country_bucket2(excel_path, country_code, donors_bucket2, all_processed):
    """
    For countries with only TOTAL capital stock available (bucket2),
    estimate 64 industry capital stock using average donor CI times VA,
    ...
    # initialize cap_64 numeric
    cap_64_num = pd.DataFrame(index=INDUSTRIES_64, columns=YEARS_STR, dtype=float)

    # For bucket2 countries, we will compute cap_64_num from donor CI averages * VA, then scale to TOTAL
    donors = donors_bucket2.get(country_code, [])
    # donors must be present in all_processed (processed earlier)
    # Build donor CI lookup: for each industry-year collect donor CI values
    for year in YEARS_STR:
        # read TOTAL for the country-year
        total_val = cap_df_num.loc["TOTAL", year] if "TOTAL" in cap_df_num.index else np.nan

        # build avg donor CI per industry
        avg_ci_by_ind = {}
        for ind in INDUSTRIES_64:
            donor_cis = []
            for d in donors:
                if d not in all_processed:
                    continue
                d_ci = all_processed[d]["CI"].get((ind, year))
                if d_ci is None or pd.isna(d_ci):
                    continue
                # filter abnormal as specified (<0 or >100)
                if d_ci < 0 or d_ci > 100:
                    continue
                donor_cis.append(d_ci)
            if len(donor_cis) == 0:
                avg_ci_by_ind[ind] = np.nan
            else:
                avg_ci_by_ind[ind] = float(np.mean(donor_cis))

        # estimate K_i = VA_i * avg_CI_i
        est_k = {}
        for ind in INDUSTRIES_64:
            va_i = va_df_num.loc[ind, year]
            ci_i = avg_ci_by_ind.get(ind, np.nan)
            if pd.isna(va_i) or pd.isna(ci_i):
                est_k[ind] = 0.0
            else:
                est_k[ind] = va_i * ci_i

        sum_est_pos = sum(v for v in est_k.values() if v > 0)

        # if total_val is numeric and sum_est_pos > 0, scale; otherwise leave NaN (i.e., keep missing)
        if not pd.isna(total_val) and sum_est_pos > 0:
            scale = total_val / sum_est_pos
            for ind in INDUSTRIES_64:
                if est_k[ind] > 0:
                    cap_64_num.loc[ind, year] = est_k[ind] * scale
                else:
                    # leave NaN to become ":" later
                    cap_64_num.loc[ind, year] = np.nan
        else:
            # cannot scale (no total or no positive estimates) -> leave NaNs
            for ind in INDUSTRIES_64:
                cap_64_num.loc[ind, year] = np.nan
...
if __name__ == "__main__":
    ...
    output_file = "/Users/nikhil/Documents/Thesis/Capital/0_PROCESSED_NetCapitalStock.xlsx"
    with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
        ...
        for cc in country_codes_bucket2:
            print(f"Processing bucket2 {cc} ...")
            out_df = process_country_bucket2(excel_path, cc, DONORS_BUCKET2, all_processed)
            out_df.to_excel(writer, sheet_name=cc, index=False)
            print(f"  → Done {cc}")
