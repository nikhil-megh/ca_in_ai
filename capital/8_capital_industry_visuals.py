...
# ---------- CONFIG ----------
INPUT_XLSX = "/Users/nikhil/Documents/Thesis/Capital/Descriptives_Industry/Capital_Industry_Summary.xlsx"
OUTPUT_DIR = "/Users/nikhil/Documents/Thesis/Capital/Descriptives_Industry/Visuals"

# Sheet constants
SHEET_GLOBAL_CAPITAL = "Global_Capital_Industry_Share"
SHEET_EU_CAPITAL = "EU_Capital_Industry_Share"
...
def generate_all_industry_charts(input_xlsx, output_dir):
    ...
    sheets_to_load = {
        "global_capital": SHEET_GLOBAL_CAPITAL,
        "eu_capital": SHEET_EU_CAPITAL
    }
    data = {}
    ...
    # --- Global Capital industry pies & trends (top 12) ---
    if data["global_capital"] is not None:
        df_global_capital = data["global_capital"].copy()
        if 2021 not in df_global_capital.columns:
            raise ValueError("Global_Capital_Industry_Share must contain a '2021' column.")
        s2021 = df_global_capital[2021].astype(float)
        s2021 = normalize_shares_series(s2021)
        pie_top_n_with_others(s2021, top_n=12,
                              title="Global Industry Capital Share 2021 (Top 12)",
                              outpath=os.path.join(output_dir, "Global_Capital_Industry_Top12_2021_pie.png"))

        line_trend_top_n(df_global_capital, top_n=12, years=TREND_YEARS,
                         title="Global Industry Capital Share Trend 2010-2021 (Top 12)",
                         outpath=os.path.join(output_dir, "Global_Capital_Industry_Top12_trend_2010_2021.png"))
    else:
        print("Skipping Global Capital industry charts (sheet missing).")

    # --- EU Capital industry pies & trends (top 12) ---
    if data["eu_capital"] is not None:
        df_eu_capital = data["eu_capital"].copy()
        if 2021 not in df_eu_capital.columns:
            raise ValueError("EU_Capital_Industry_Share must contain a '2021' column.")
        s2021_eu = df_eu_capital[2021].astype(float)
        s2021_eu = normalize_shares_series(s2021_eu)
        pie_top_n_with_others(s2021_eu, top_n=12,
                              title="EU Industry Capital Share 2021 (Top 12)",
                              outpath=os.path.join(output_dir, "EU_Capital_Industry_Top12_2021_pie.png"))

        line_trend_top_n(df_eu_capital, top_n=12, years=TREND_YEARS,
                         title="EU Industry Capital Share Trend 2010-2021 (Top 12)",
                         outpath=os.path.join(output_dir, "EU_Capital_Industry_Top12_trend_2010_2021.png"))
    else:
        print("Skipping EU Capital industry charts (sheet missing).")
